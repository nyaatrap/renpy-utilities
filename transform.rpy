...
##############################################################################
## Positions
## 以下の transform は画像を表示する位置を定義します。
## ほとんどはデフォルトで定義されていますが、
## 表示済みの画像の座標を変える時には、時間を掛けて移動するように拡張しています。


init python:
    ## 座標の移動にかかる時間をまとめて定義します。
    base_atl_time = .5


## 中央に下揃えで表示する。
transform center(t=base_atl_time):
    subpixel True
    on show:
...
## 左に下揃えで表示する。
transform left(t=base_atl_time):
    subpixel True
    on show:
...
## 右に下揃えで表示する。
transform right(t=base_atl_time):
    subpixel True
    on show:
...
## 画面の左外に下揃えで表示する。
transform offscreenleft(t=base_atl_time):
    subpixel True
    on show:
...
## 画面の右外に下揃えで表示する。
transform offscreenright(t=base_atl_time):
    subpixel True
    on show:
...
## 中央に表示する。
transform truecenter(t=base_atl_time):
    subpixel True
    on show:
        anchor (.5, .5) pos (.5, .5)
    on replace:
...
## 左に表示する。
transform trueleft(t=base_atl_time):
    subpixel True
    on show:
        anchor (.0, .5) pos (.0, .5)
    on replace:
...
## 右に表示する。
transform trueright(t=base_atl_time):
    subpixel True
    on show:
...
transform zoom(z=1.0, t=base_atl_time):
    subpixel True
    on show:
        zoom z
    on replace:
...
## 使用頻度が低い為ここでは再定義していません。必要の場合は上記を参考に定義してください。
...
init python:
    ## アニメーションの動きの大きさを決めます。
    ## 画面の縦座標の30分の１くらいが目安です。
    base_atl_offset =24


## 軽くホップするアニメーション。
transform hop(t=base_atl_time/2, d=base_atl_offset):
    alpha 1.0
    ease t/2 yoffset -d
    ease t/2 yoffset 0

## 二回連続してホップするアニメーション。
transform doublehop(t=base_atl_time, d=base_atl_offset):
    alpha 1.0
    ease t/4 yoffset -d
    ease t/4 yoffset 0
    ease t/4 yoffset -d
...
## 軽くかがむアニメーション。
transform bow(t=base_atl_time, d=base_atl_offset):
    alpha 1.0
    ease t/2 yoffset d
    ease t/2 yoffset 0

## 左右に二回揺れるアニメーション。
transform wag(t=base_atl_time, d=base_atl_offset):
    alpha 1.0
...
## 素早く右に避けるアニメーション。
transform sway_r(t=base_atl_time/2, d=base_atl_offset):
    alpha 1.0
    ease t/3 xoffset d*4
    ease t*2/3 xoffset 0

## 素早く左に避けるアニメーション。
transform sway_l(t=base_atl_time/2, d=base_atl_offset):
    alpha 1.0
    ease t/3 xoffset -d*4
    ease t*2/3 xoffset 0
...
## 実際の振動アニメーション。
transform shake(t=base_atl_time, d=base_atl_offset):
    alpha 1.0
